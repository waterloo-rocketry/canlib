...
w_status_t get_stream_status(const can_msg_t *msg, uint32_t *total_size, uint32_t *tx_size) {
	w_assert(msg);
	w_assert(total_size);
	w_assert(tx_size);
	...
	*tx_size = ((uint32_t)msg->data[5] << 16) | ((uint32_t)msg->data[6] << 8) | msg->data[7];
	...
	if (get_message_type(msg) != MSG_STREAM_STATUS) {
...
w_status_t get_stream_data(const can_msg_t *msg, uint8_t *seq_id, uint8_t *payload,
	...
	*payload_len = msg_payload_len;
	...
	if (get_message_type(msg) != MSG_STREAM_DATA) {
		return W_INVALID_PARAM;
	}

	if ((msg->data_len < 3) || (msg->data_len > (2 + STREAM_DATA_MAX_PAYLOAD_LEN))) {
		return W_DATA_FORMAT_ERROR;
	}
...
w_status_t get_stream_retry_seq_id(const can_msg_t *msg, uint8_t *seq_id) {
	w_assert(msg);
	w_assert(seq_id);

	*seq_id = get_message_metadata(msg);

	if (get_message_type(msg) != MSG_STREAM_RETRY) {
		return W_INVALID_PARAM;
	}
