...
w_status_t get_gps_alt(const can_msg_t *msg, uint32_t *altitude, uint8_t *daltitude) {
	w_assert(msg);
	w_assert(altitude);
	w_assert(daltitude);

	*altitude = ((uint32_t)msg->data[2] << 24) | ((uint32_t)msg->data[3] << 16) |
				((uint32_t)msg->data[4] << 8) | msg->data[5];
	*daltitude = msg->data[6];
...
w_status_t get_gps_info(const can_msg_t *msg, uint8_t *num_sat, uint8_t *quality) {
	w_assert(msg);
	w_assert(num_sat);
	w_assert(quality);
