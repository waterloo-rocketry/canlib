...
public:
	...
	bool run_test() override {
		...
		build_telemetry_info_msg(prio_before, timestamp_before, channel_id_before, lqi_before,
		                         rssi_before, &msg);
		...
		rockettest_check_expr_true(get_telemetry_info_msg(&invalid_type_msg, &channel_id_after,
		                                                  &lqi_after,
		                                                  &rssi_after) == W_INVALID_PARAM);

		can_msg_t invalid_len_msg = msg;
		invalid_len_msg.data_len = 3;
		rockettest_check_expr_true(get_telemetry_info_msg(&invalid_len_msg, &channel_id_after,
		                                                  &lqi_after,
		                                                  &rssi_after) == W_DATA_FORMAT_ERROR);
