...
public:
	...
	bool run_test() override {
		...
		build_general_board_status_msg(prio_before,
									   timestamp_before,
									   general_error_bitfield_before,
									   board_error_bitfield_before,
									   &msg);
		...
		board_error_bitfield_extracted =
			(static_cast<std::uint16_t>(msg.data[6]) << 8) | msg.data[7];

		rockettest_check_expr_true(msg.data_len == 8);
		rockettest_check_expr_true(timestamp_extracted == timestamp_before);
		rockettest_check_expr_true(general_error_bitfield_extracted ==
								   general_error_bitfield_before);
