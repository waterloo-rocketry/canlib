...
public:
	...
	bool run_test() override {
		...
		build_canard_firmware_error_msg(prio_before, timestamp_before, module_id_before,
		                                error_bitfield_before, severity_before, &msg);
		...
		rockettest_check_expr_true(get_canard_firmware_error_msg(&msg, &module_id_after,
		                                                         &error_bitfield_after,
		                                                         &severity_after) == W_SUCCESS);
		...
		rockettest_check_expr_true(get_canard_firmware_error_msg(
									   &invalid_type_msg, &module_id_after, &error_bitfield_after,
									   &severity_after) == W_INVALID_PARAM);

		can_msg_t invalid_len_msg = msg;
		invalid_len_msg.data_len = 3;
		rockettest_check_expr_true(
			get_canard_firmware_error_msg(&invalid_len_msg, &module_id_after, &error_bitfield_after,
			                              &severity_after) == W_DATA_FORMAT_ERROR);
